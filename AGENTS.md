...
- 증상 필터는 병원을 **탈락시키지 않고 3단으로 분류**한다. 수용정보가 비어있어 무조건 탈락시키면 대부분의 병원이 사라져 "정보 미보고"와 "수용 불가"가 구분되지 않는 문제가 있었다.
  - `matched`(수용 가능): 선택한 모든 그룹을 수용 가능하다고 보고.
  - `unavailable`(수용 불가): 수용정보를 보고했으나 일부 그룹 미충족.
