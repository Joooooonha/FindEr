...
- 의료/응급 데이터는 사용자 판단에 직접 영향을 주므로, 최신성·출처·불확실성을 UI/API에서 명확히 드러낸다.
...
한 줄 정의: **FindEr는 응급실 진료가 필요한 사람이 주변 응급실 정보를 지도에서 비교할 수 있게 하는 서비스다.**

왜 만드는가:

- 타겟 상황: 위급한 상황에 응급실 진료가 필요한 환자/보호자. 즉시 도움이 필요한 경우에는 119 안내를 우선한다.
...
- 증상별/병상별 정보는 사용자가 빠르게 비교할 수 있게 병상 수, 상태, 갱신 시각, 가능 시술 정보를 함께 표시한다.
...
- 오래되거나 비정상인 병상정보를 백엔드와 프론트엔드 양쪽에서 `UNKNOWN`과 `정보 없음`으로 명확히 구분하는 방어 로직 보강.
...
- 유효한 병상 수와 갱신 시각은 의료 판단에 중요하므로 함께 제공한다. 유효하지 않은 병상 수는 아래 규칙에 따라 `정보 없음`으로 명확히 표시한다.
- `availableEmergencyBeds`가 음수이거나 갱신이 30분을 초과한 데이터는 `UNKNOWN` 처리한다.
- `UNKNOWN` 상태에서는 병상 수를 `정보 없음`으로 표시한다.
