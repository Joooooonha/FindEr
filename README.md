...
| 상태 | 웹 배포 경험 완료 · 데이터 신뢰성 및 문서 개선 중 |
...
처음에는 병상 정보 접근성이 응급실 뺑뺑이의 핵심 문제라고 보았습니다. 분산된 정보를 하나의 API와 지도 화면으로 결합해 배포했지만, 이후 원인을 다시 조사하면서 의료 인력·병상·진료 전달 체계가 얽힌 구조적 문제라는 점을 확인했습니다.

이 프로젝트가 해결한 것은 **주변 응급실 정보를 비교하는 과정의 불편**이지, 병원 수용을 보장하거나 응급의료 체계의 병목을 해결하는 일이 아닙니다. 이 차이를 뒤늦게 분명히 한 경험 이후에는 기능을 바로 구현하기 전에 다음 질문을 먼저 확인하게 됐습니다.
